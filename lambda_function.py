...
def get_welcome_response(event):
    ...
    if event['request']['locale'] == 'en-GB' and 'PLAY_ADVERT' in environ and randint(1,10) == 10:
        advert1 = '<voice name="Brian"><prosody rate="fast">Do you like smart home tech if so check out paul hibbert? '
...
def search(event):
    ...
    if next_url == False:
        return build_response(build_short_speechlet_response('youtube is talking to me', True))
